  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-900">
    <header class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
      <div class="max-w-4xl mx-auto px-4 py-6">
        ...
        <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100 mt-4">所有工具</h1>
        <p class="text-lg text-slate-500 dark:text-slate-400 mt-2">共 {{ sites.length }} 个趋势监控工具</p>
      </div>
    </header>
    ...
    <main class="max-w-4xl mx-auto px-4 py-8">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <NuxtLink
          ...
          :key="site.id"
          :to="`/sites/${site.id}`"
          class="block bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all hover:-translate-y-1"
        >
          <div class="flex items-center gap-3 mb-3">
            ...
            <h2 class="font-semibold text-slate-900 dark:text-slate-100">{{ site.name }}</h2>
          </div>
          <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2">{{ site.description }}</p>
