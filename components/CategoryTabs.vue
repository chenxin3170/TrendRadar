...
const props = defineProps<{
  modelValue: CategoryId | 'all' | 'favorites'
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: CategoryId | 'all' | 'favorites'): void
}>()
...
const favoritesTab = {
  ...
  name: '我的收藏',
  nameEn: 'Favorites',
  icon: categoryIcons.favorites
}

const selectCategory = (category: CategoryId | 'all' | 'favorites') => {
  emit('update:modelValue', category)
}
</script>

<template>
  <div class="flex overflow-x-auto gap-2 mb-8 pb-2 snap-x snap-mandatory hide-scrollbar">
    <button
      v-for="cat in [allTab, favoritesTab, ...categories]"
      :key="cat.id"
      @click="selectCategory(cat.id)"
      class="shrink-0 snap-start inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 whitespace-nowrap"
      :class="[
        modelValue === cat.id
    ...
    >
      <svg
        ...
        viewBox="0 0 24 24"
      >
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="categoryIcons[cat.id] || allTab.icon" />
      </svg>
